// Server-canonical agent archetypes. These are the source of truth for the
// "change of soul" (re-role) feature: a free-tier user picks a preset by id and
// the server applies THIS soul text — so free users can re-role between presets
// but cannot author a custom soul (that's a Standard-tier capability). The
// client gallery in app/src/lib/agentTemplates.ts mirrors these for display;
